...
function ChatPanel({ messages }: { messages: Message[] }) {
  return (
    <div className="flex-1 px-6 py-8 overflow-y-auto space-y-6 scrollbar-thin">
      {messages.map((msg) => (
        <div
          key={msg.id}
        ...
        >
          <div
            className={`rounded-2xl px-4 py-3 text-sm leading-relaxed max-w-[75%] shadow-lg backdrop-blur-sm ${
              msg.author === "user"
...
function ChatInput({
  ...
  return (
    <footer className="px-6 py-5 bg-neutral-900/30 backdrop-blur-sm border-t border-neutral-800/50">
